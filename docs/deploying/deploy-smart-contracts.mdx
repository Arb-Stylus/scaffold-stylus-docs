...
> **Note:** If you encounter an `execution reverted` error when deploying larger contracts locally, see the [Devnode Troubleshooting](#devnode-troubleshooting) section below.
...
## Devnode Troubleshooting

### `execution reverted` on Contract Activation
...
**Why this happens:**
Stylus contracts larger than 24KB (24576 bytes compressed) are deployed as **multiple fragments**. Activating a multi-fragment contract requires ArbOS 60. A devnode running an older nitro image boots at ArbOS 59 and the activation reverts, with an error that does not obviously point at ArbOS.
